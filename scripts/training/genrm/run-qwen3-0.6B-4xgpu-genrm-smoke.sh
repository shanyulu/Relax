...
#
# Minimal native-entrypoint smoke for the GenRM elastic-scaling validation.
#
# It deliberately validates one real DAPO + GenRM training path rather than
# serving as a throughput recipe.  The 2 + 1 + 1 topology leaves two GPUs for
# Megatron actor training, one for rollout, and one for the GenRM engine.
# KL is omitted: enabling it would require a non-zero-GPU reference/actor_fwd
# topology and would test a different concern from this smoke.
#
...
#     MODEL_PATH=... PROMPT_SET=... RAY_NO_WAIT=1 \
#     bash scripts/entrypoint/ray-job.sh scripts/training/genrm/run-qwen3-0.6B-4xgpu-genrm-smoke.sh
...
SAVE_DIR="${SAVE_DIR:-${SCRIPT_DIR}/../../../checkpoints/task4-genrm-smoke}"
