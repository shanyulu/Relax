...
**2026-09-25 估计量统一与合并统计**（[分析脚本与结果](https://github.com/shanyulu/Relax/tree/a5813f6de4f995cf8d883caab59cecee95298358/demos/task11_straggler/results/multisession-20260924)）：以配对中位数为点估计，用 session-aware 层级 bootstrap（先重采样会话、再重采样会话内配对）对**同一统计量**取区间，另报整段墙钟增量与单对尾部：

| 样本池 | 配对中位数 | 中位数层级区间 95% | 均值层级区间 95% | 最大单对 | 整段墙钟增量 |
| --- | --- | --- | --- | --- | --- |
| 第一批 off/on（16 对） | +0.0861% | [+0.0017%, +0.2536%] | [+0.0621%, +0.2958%] | +1.035% | +0.1632% |
| 第二批 off/on（16 对） | +0.1844% | [+0.0762%, +0.2963%] | [+0.0875%, +0.4812%] | +2.548% | +0.2824% |
| 同批 A/A（16 对） | +0.0135% | [−0.3890%, +0.4767%] | [−0.3492%, +0.1512%] | +0.551% | — |

统一估计量后各区间上界低于 0.5%，但**不据此改判通过**：历史口径的逐对均值区间上界仍为 +0.7061%；4 个会话只有 4 个聚类，层级区间粒度粗（约 256 种重采样组合）；+2.5% 的单对说明低频昂贵回合存在，正是中位数会掩盖的成本。验收口径（中位数区间还是均值区间）请导师裁决，recipe 级实测仍是最终依据。逐步阶段耗时（bench-on 段，仅 on 臂有数据）p50≈2.45ms、p95≈2.50ms、p99≈2.5–3.2ms、max 3.8–5.3ms；上报滞后峰值 103–107ms。

干净负载段告警带分母（两批同口径：bench-on 段告警数 ÷ 该段完整 cohort-window 数）：第一批 2/1/2/0（各 ÷4,000 → 0.050%/0.025%/0.050%/0%），第二批 14/0/0/0（÷4,000 → A 会话 0.35%），合并 19/32,000 = 0.059%；告警 cause 均为 `undetermined`，不能排除真实 host stall，故不标记为“误报”。第二批每会话收到 24,160/24,160 个样本，无丢弃、loss 差异或参数错配；这些仍只是该 standalone 工作负载的结果。
