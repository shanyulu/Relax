...
| [A/A 对照会话](https://github.com/shanyulu/Relax/tree/1e29727dd38e4b5086e01d7e97e6d31d47a29e7/demos/task11_straggler/results/multisession-20260924)（2026-09-24 补跑）                                                                                                                                                    | 同协议 4 会话、各含 4 对双臂全开采集的 A/A 对照（每会话 24,160/24,160 样本、零丢弃、零 loss 差异、零参数错配）：A/A 合并中位数 +0.0135%、bootstrap mean 95% [−0.333%, +0.173%]，而 off/off 空对照漂移最高 +3.00%——观测器自身计时偏差与零不可区分，测量散布以环境漂移为主；A/A 会话 16 对 off/on 开销合并中位数 +0.1844%、bootstrap 上界 +0.7061% 仍高于 0.5%（A/A 对本身为观测噪声、不计入开销池），验收计划（recipe 实测）不变 |
...
下图由 multisession-20260924 的 8 份会话原始 JSON 生成（绘制脚本与数据同目录）：(a) 两个批次各自合并的 off/on 配对开销，A/A 批合并上界 0.706% 仍高于 0.5%；(b) A/A 合并中位数 +0.014% 且 CI 过零——观测器无偏，而 off/off 空对照漂移达 +3.0%，说明会话间环境漂移才是测量散布的主导项。

![多会话森林图：off/on 开销（两批次各自合并）与 A/A≈0 / off/off 漂移的偏置对照](https://raw.githubusercontent.com/shanyulu/Relax/d469e491f50c89e6fb006897ddd39c91536ace86/demos/task11_straggler/results/multisession-20260924/multisession-forest.png)
